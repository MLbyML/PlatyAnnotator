...
public class MyOverlay extends BdvOverlay {
    ...
    private Color getColor(final Point point) {
        ...
        final int b = point.getIntPosition(2);
    ...
    public void createImage(String absolutePath, long[] dimensions) {
        ...
        for (int i = 0; i < thresholdedLocalMinima.size(); i++) {
            ...
            if (zCenter != -1) {
                HyperSphere<UnsignedByteType> sphereRed = new HyperSphere<>(outputRed, new Point(xCenter, yCenter, zCenter), radius);
                net.imglib2.Cursor<UnsignedByteType> cursorRed = sphereRed.localizingCursor();
                HyperSphere<UnsignedByteType> sphereGreen = new HyperSphere<>(outputGreen, new Point(xCenter, yCenter, zCenter), radius);
                net.imglib2.Cursor<UnsignedByteType> cursorGreen = sphereGreen.localizingCursor();
                HyperSphere<UnsignedByteType> sphereBlue = new HyperSphere<>(outputBlue, new Point(xCenter, yCenter, zCenter), radius);
                ...
                HyperSphere<UnsignedByteType> sphereRed = new HyperSphere<>(outputRed, new Point(xCenter, yCenter), radius);
                net.imglib2.Cursor<UnsignedByteType> cursorRed = sphereRed.localizingCursor();
                HyperSphere<UnsignedByteType> sphereGreen = new HyperSphere<>(outputGreen, new Point(xCenter, yCenter), radius);
                net.imglib2.Cursor<UnsignedByteType> cursorGreen = sphereGreen.localizingCursor();
                HyperSphere<UnsignedByteType> sphereBlue = new HyperSphere<>(outputBlue, new Point(xCenter, yCenter), radius);
                net.imglib2.Cursor<UnsignedByteType> cursorBlue = sphereBlue.localizingCursor();
                while (cursorRed.hasNext()) {
                    cursorRed.fwd();
                    cursorRed.get().set(thresholdedLocalMinima.get(i).getRed());
